...
GtkWidget *username_entry, *password_entry, *retypePassword_entry, *fullname_entry; //in function register_dialog_show
...
char fullNameFile[MAX_LETTER]; //in finction login

int stop_loop_Main = 0; //0=false, 1 = true
...
//gọi hàm khi nhấn button
  g_signal_connect(window,"destroy",G_CALLBACK(exit_screen),NULL); // tắt app 
...
int checkUserName(const char *userNameTmp)
{
...
int checkPassword(const char *passwordTmp)    // mat khau phai co tu 8 ki tu tro len va co chu hoa va chu thuong
{
...
int signUp() {
    const char *userNameTmp = gtk_entry_get_text(GTK_ENTRY(username_entry)); // cac mang tam
    ...
    if(checkUserName(userNameTmp) == 0) { //kiem tra xem username da ton tai hay chua
        if(checkPassword(passwordTmp) == 0) { // kiem tra xem password dung format hay chua
            if( strcasecmp(retypePassword, passwordTmp) == 0) { // kiem tra xem retype = type pass hay chua
                register_success();
...
// gcc `pkg-config --cflags gtk+-3.0` -o app final.c `pkg-config --libs gtk+-3.0`
// ./app
