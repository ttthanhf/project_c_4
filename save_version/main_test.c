...
char fullNameFile[1000];
const char *fullNameTmp;
int indexOfUser;                                                                                                                                    // dùng để lưu lại số thứ tự user
int stop_loop_Main = 0;                                                                                                                             // 0=false, 1 = true                                                                                                                                                                                                                                              // in function signUp
...
int checkUserName(const char *userNameTmp) {
  ...
  char userNameFile_username[1000];
  char line_username[1000];
  int check = 0;
  ...
  fflush(file_username);
  fseek(file_username, 0, SEEK_SET);

    if (strcasecmp(userNameTmp, "") != 0) {
      while (fgets(line_username, sizeof(line_username), file_username)) {

        fscanf(file_username, "Name: %s", &userNameFile_username);

        // printf("%s",userNameFile_username);
        if (strcmp(userNameTmp, userNameFile_username) != 0) {
          return 1;
          break;
        }
        else {
...
void signUp() {
    ...
    const char *userNameTmp = gtk_entry_get_text(GTK_ENTRY(username_entry));
...
void login() {
    ...
    while (fgets(line, sizeof(line), file_login)) {
        ...
        if (strcmp(userNameTmp, userNameFile) == 0 && strcmp(passwordFile, passwordTmp) == 0) {
